public class Frequency_Generator {

	public Frequency_Generator() {

	}

	public static float[] generate_Freqs(int octaves, int notesPerOctave, float rootNote) {
		// make an array of notes to be populated
		float[] notes = new float[notesPerOctave * octaves];
		// set the root note
		notes[0] = rootNote;
		// find the interval ratio
		float intervalRatio = (float) nth_Root(notesPerOctave, 2);
	...
	public static float nth_Root(int n, float x) {
		if (x < 0) {
